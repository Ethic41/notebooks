...
class CrudUtil:
    ...
    async def list_model(
        ...
        try:
            ...
            return PagedResult[T](
                data=model_list, count=db_model_count, sum=Decimal(sum_total or 0)
            )
    ...
    async def search_model(
        ...
        for column in search_columns:
            is_str_col: bool = (
                model_to_search.__table__.columns[column].type.python_type is str  # type: ignore
            )
            if is_str_col:
        ...
        # Execution
        result: Result[Any] = await self.db.execute(query)
        model_list: Any = result.unique().scalars().all()
        ...
        return PagedResult[T](data=model_list, count=db_model_count, sum=None)
